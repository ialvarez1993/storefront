...
export const useWishlist = () => {
  ...
  const loadWishlist = async () => {
    ...
    wishlistCounter.value = Number(data?.value?.wishlistItems?.wishlistItems?.length || 0)
  }
  ...
  const wishlistAddItem = async (productId: number) => {
    ...
    wishlistCounter.value = Number(data?.value?.wishlistAddItem?.wishlistItems?.length || 0)
  }
  ...
  const wishlistRemoveItem = async (productId: number) => {
    ...
    const { data } = await $sdk().odoo.mutation<
      MutationWishlistRemoveItemArgs,
      WishlistRemoveItemResponse
    >({ mutationName: MutationName.WishlistRemoveItem }, { wishId: wishlistItem.id })
    loading.value = false

    wishlist.value = data?.value.wishlistRemoveItem
    wishlistCounter.value = Number(data?.value?.wishlistRemoveItem?.wishlistItems?.length || 0)
  }
