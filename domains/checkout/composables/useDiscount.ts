...
export const useDiscount = () => {
  ...
  const applyGiftCard = async (promo: MutationApplyGiftCardArgs) => {
    return $sdk().odoo.mutation<MutationApplyGiftCardArgs, ApplyDiscountsResponse>(
      { mutationName: MutationName.ApplyGiftCardMutation },
      promo
    )
  }

  const applyCoupon = async (promo: MutationApplyCouponArgs) => {
    return $sdk().odoo.mutation<MutationApplyCouponArgs, ApplyDiscountsResponse>(
  ...
  const makeGiftCardPayment = async () => {
    ...
    const { data, error } = await $sdk().odoo.mutation<null, MakeGiftCardPaymentResponse>({
