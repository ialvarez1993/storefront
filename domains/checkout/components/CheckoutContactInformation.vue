  ...
  <div data-testid="checkout-address" class="md:px-4 py-6">
    ...
    <transition enter-active-class="transition duration-200 ease-out"
      ...
      <SfModal v-model="isOpen" tag="section" role="dialog"
        class="h-full w-full overflow-auto md:w-[600px] md:h-fit z-50" aria-labelledby="contact-modal-title">
        <header>
        ...
        <form data-testid="contact-information-form" @submit.prevent="handleUpdatePartnerData">
          <label>
            ...
            <SfInput v-model="name" name="name" type="text" :placeholder="$t('contactInfo.name')" required />
          </label>
          <div class="mt-4" />
          <label>
            <UiFormLabel>{{ $t("contactInfo.email") }}</UiFormLabel>
            <SfInput v-model="email" name="email" type="email" :placeholder="$t('contactInfo.email')" required />
          </label>
          <div class="mt-4">
            <label>
              <UiFormLabel>{{ $t("contactInfo.subescribe") }}</UiFormLabel>
              <SfSwitch class="" v-model="subscribeNewsletter" />
            </label>
          </div>
