...
<template>
  ...
  <SfScrollable v-if="productTemplateList.length > 0" buttons-placement="floating" class="items-center pb-4"
    data-testid="product-slider">
    <LazyUiProductCard v-for="productTemplate in productTemplateList" :key="productTemplate.id" class="min-w-[190px]"
      :slug="mountUrlSlugForProductVariant(
        productTemplate.firstVariant as Product,
      ) || ''
        " :name="productTemplate?.name || ''" :image-url="$getImage(
      ...
      :is-in-wishlist="productTemplate?.isInWishlist || false" :rating-count="productTemplate.ratingCount"
      :rating="productTemplate.rating" :first-variant="productTemplate.firstVariant as Product" />
  </SfScrollable>
</template>
