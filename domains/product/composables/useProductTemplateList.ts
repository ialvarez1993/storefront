...
export const useProductTemplateList = (categorySlugIndex?: string, fullSearchIndex?: string) => {
  const { $sdk } = useNuxtApp()

  const loading = useState(`loading-product-template-list-${fullSearchIndex}`, () => false)
  const totalItems = useState<number>(`total-items${fullSearchIndex}`, () => 0)
  const productTemplateList = useState<Product[]>(`products-category${fullSearchIndex}`, () => [])
  const attributes = useState<AttributeValue[]>(`attributes${categorySlugIndex}`, () => [])
  const categories = useState<Category[]>(`categories-from-product-${categorySlugIndex}`, () => [])

  const loadProductTemplateList = async (params: QueryProductsArgs, force: boolean = false) => {
  ...
  const organizedAttributes = computed(() => {
    ...
    attributes.value?.forEach((item: any) => {
      const current = data.find(
        (itemData: { attributeName: any }) => itemData.attributeName === item.attribute?.name
      )
