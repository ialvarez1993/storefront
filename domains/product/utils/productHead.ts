...
const validateproductSEO = (product, fullPath) => {
  ...
  if (!product?.metaTitle) {
    warnings.push(`[WARNING] - The product from slug ${fullPath} does not have the metaTitle.`)
  }
  ...
  if (!product?.jsonLd) {
    warnings.push(`[WARNING] - The product from slug ${fullPath} does not have the jsonLd.`)
  }
  if (!product?.metaKeyword) {
    warnings.push(`[WARNING] - The product from slug ${fullPath} does not have the metaKeyword.`)
  }
  if (!product?.metaImage) {
    warnings.push(`[WARNING] - The product from slug ${fullPath} does not have the metaImage.`)
  }
