...
<template>
  <form class="grid grid-cols-1 md:grid-cols-[50%_1fr_120px] gap-4" data-testid="address-form"
    ...
    <UiFormSelectStates v-model="addressFormFieldsInput.stateId" :key="addressFormFieldsInput?.countryId"
    ...
    <div class="md:col-span-3 flex flex-col-reverse md:flex-row justify-end mt-6 gap-4">
      <SfButton type="reset" class="bg-black text-white" variant="secondary" @click="$emit('on-close')">
        {{ $t("contactInfo.cancel") }}
      </SfButton>
      <SfButton type="submit" class="text-black bg-yellow-400 min-w-[120px]" :disabled="loading">
