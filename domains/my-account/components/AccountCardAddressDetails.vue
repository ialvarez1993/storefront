...
<template>
  <div class="md:col-span-1 col-span-3" v-for="address in addresses" :key="address.id">
    <AccountAddressData @on-click="handleOpenFormForEditAddress(address)" :header="address.name || ''"
  ...
  <div class="col-span-3">
    <SfButton size="lg" @click="handleOpenFormToAddAddress()" class="bg-black self-start">Add new address</SfButton>
  </div>
  <UiOverlay v-if="isOpen" :visible="isOpen">
    <UiModal :model-value="isOpen" tag="section" role="dialog"
      ...
      <AccountFormAddress :is-edit-form="edit" :address="addressForEdit" :type="type"
        @on-save="handleCloseAfterSaveAddress" @on-close="close" />
    </UiModal>
  </UiOverlay>
</template>
