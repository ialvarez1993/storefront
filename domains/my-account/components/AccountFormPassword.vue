...
<template>
  <form data-testid="account-forms-password" @submit.prevent="$emit('on-save', userPasswords)">
    <label class="block">
      ...
      <SfInput v-model="userPasswords.oldPassword" name="password" :type="passwordVisible ? 'text' : 'password'"
        required>
        <template #suffix>
    ...
    <label class="block my-4">
      ...
      <SfInput v-model="userPasswords.firstNewPassword" name="password"
        :type="firstNewPasswordVisible ? 'text' : 'password'" required>
        <template #suffix>
    ...
    <label class="block">
      ...
      <SfInput v-model="userPasswords.secondNewPassword" name="password"
        :type="secondNewPasswordVisible ? 'text' : 'password'" required>
        <template #suffix>
    ...
    <div class="mt-6 flex flex-col-reverse md:flex-row md:justify-end gap-4">
      <SfButton class="!bg-black text-white" type="reset" variant="secondary" @click="$emit('on-cancel')">
        {{ $t("contactInfo.cancel") }}
      </SfButton>
      <SfButton type="submit" class="bg-yellow-400 min-w-[120px]">
