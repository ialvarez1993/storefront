...
<template>
  <div v-if="cart?.order?.websiteOrderLine?.length > 0" class="pb-20 !mx-[5rem] !mt-[14rem]">
      ...
      <div class="col-span-5 md:sticky md:top-20 h-fit">
        ...
        <UiOrderSummary :cart="cart">
          <NuxtLink to="/checkout">
            <SfButton size="lg" class="w-full !bg-black mb-4 md:mb-0">
              {{ $t("goToCheckout") }}
            </SfButton>
          </NuxtLink>
        </UiOrderSummary>
  ...
  <div v-else class="flex items-center justify-center mt-36 flex-col pt-24 pb-32" data-testid="cart-page-content">
