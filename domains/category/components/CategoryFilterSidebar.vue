...
const sort = useState('sort', () => (route.query?.sort ? route.query?.sort : ''))
...
watch(priceModel, (newValue) => {
  selectedFilters.value = selectedFilters.value.filter((item: any) => item.filterName !== 'price')
  if (newValue) {
    selectedFilters.value.push({
      filterName: 'price',
      id: newValue
  ...
  <aside class="w-full lg:max-w-[376px]">
    <template v-if="categories.length">
      ...
      <ul class="mt-4 mb-6 md:mt-2" data-testid="categories">
        ...
        >
          <span class="flex gap-2 items-center" @click="changeCategory(category.id)">
            <span
        ...
        <SfAccordionItem v-model="opened[facet.label]">
          <template #summary>
            <div class="flex justify-between items-center p-2 mb-2">
              ...
              <SfIconChevronLeft :class="opened[facet.label] ? 'rotate-90' : '-rotate-90'" />
            </div>
          </template>
    ...
    <div class="flex flex-col lg:flex-row gap-y-4 lg:gap-y-0 lg:justify-between px-3 lg:px-0">
      ...
      <SfButton class="w-full" @click="applyFilters">{{ $t('showProducts') }}</SfButton>
    </div>
  </aside>
</template>
