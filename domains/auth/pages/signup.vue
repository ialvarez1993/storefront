...
<template>
  ...
  <NuxtLayout name="auth" :heading="$t('auth.signup.heading')">
    ...
    <form data-testid="signup-form" class="flex flex-col md:border md:border-neutral-200 rounded-md gap-4 md:p-6"
      ...
      <div class="flex items-center">
        <SfCheckbox id="terms" v-model="termsAndConditionsModel" value="value" class="peer checked:bg-yellow-500 "
          required />
        <label class="ml-3 text-base text-neutral-900 cursor-pointer font-body peer-disabled:text-disabled-900"
          for="terms">
          *
      ...
      <div class="flex mb-2">
        <SfCheckbox id="subscription" v-model="subscriptionsModel" value="value"
          class="peer mt-0.5 checked:bg-yellow-500" />
        <label class="ml-3 text-base text-neutral-900 cursor-pointer font-body peer-disabled:text-disabled-900"
          for="subscription">
      ...
      <SfButton type="submit" size="lg" class="w-full !bg-black">
        {{ $t("auth.signup.createButton") }}
      </SfButton>
    </form>

    <UiModal v-model="isOpen" class="max-w-[480px] inset-x-4 md:inset-x-0" tag="section" role="alertdialog"
      disable-click-away aria-labelledby="signUpModalTitle" aria-describedby="signUpModalDesc">
      <header class="flex items-center flex-col">
        <NuxtImg src="/images/signup-success.svg" :alt="$t('auth.signup.modal.imageAlt')" width="192" height="192"
          class="my-6" />
      ...
      <UiAlert class="w-full p-4 mb-6 !justify-start typography-text-base" variant="neutral">
        <i18n-t id="signUpModalDesc" keypath="auth.signup.modal.description" tag="p">
          <template #information>
            <SfLink :tag="NuxtLink" to="/my-account"
              class="focus:outline focus:outline-offset-2 focus:outline-2 outline-secondary-600 rounded">
