...
const validateSEO = (homepage: Homepage, fullPath: string) => {
  ...
  if (!homepage?.metaTitle) {
    warnings.push(`[WARNING] - The homepage from slug ${fullPath} does not have the metaTitle.`)
  }
  ...
  if (!homepage?.jsonLd) {
    warnings.push(`[WARNING] - The homepage from slug ${fullPath} does not have the jsonLd.`)
  }
  if (!homepage?.metaKeyword) {
    warnings.push(`[WARNING] - The homepage from slug ${fullPath} does not have the metaKeyword.`)
  }
  if (!homepage?.metaImage) {
    warnings.push(`[WARNING] - The homepage from slug ${fullPath} does not have the metaImage.`)
  }
...
export default (homepage: Homepage, fullPath: string) => {
  ...
  return {
    ...
    ].filter((meta): meta is { hid: string; name: string; content: string } => !!meta), // Filtro para objetos validos válidos
    script: [
      homepage?.jsonLd && {
