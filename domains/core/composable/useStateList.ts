...
import { type State, type CountryFilterInput, type StatesResponse } from '~/graphql'
...
export const useStateList = (countryId: number) => {
  ...
  const loadStates = async () => {
    ...
    const { data } = await $sdk().odoo.query<CountryFilterInput, StatesResponse>(
      { queryName: QueryName.GetStates },
      param.value
    )
