...
const fetchMenuData = async () => {
  try {
    ...
    const data = await $fetchApi(
      `/api/inicio-menu-categorias-de-busquedas?locale=${currentLang === "es" ? "es-VE" : "en"}`,
    );
...
onMounted(async () => {
  ...
  console.log("MENU1=", menuDataResponse);
  ...
  if (menuDataResponse && menuDataResponse.length >= 3) {
    items.value[0].items[0][0].items[0] = {
      ...
      labelLink: menuDataResponse[0].linkCategory,
    ...
    items.value[0].items[1][0].items[0] = {
      ...
      labelLink: menuDataResponse[1].linkCategory,
    ...
    items.value[0].items[2][0].items[0] = {
      ...
      labelLink: menuDataResponse[2].linkCategory,
