  ...
  <nav data-testid="breadcrumbs" class="inline-flex items-center text-sm font-normal font-body">
    <ol class="flex w-auto leading-none group md:flex-wrap pl-0">
        ...
        <SfDropdown v-model="dropdownOpened" strategy="absolute" placement="bottom-start" @update:model-value="close">
          <template #trigger>
            <SfButton
              ...
              aria-label="Show more breadcrumbs" type="button" variant="tertiary" square
              data-testid="breadcrumbs-dropdown-button" @click="toggle">
              <template #prefix>
          ...
          <ol class="px-4 py-2 rounded-md shadow-md border-neutral-100" data-testid="breadcrumbs-dropdown">
            <li v-for="item in breadcrumbs" :key="item.name" class="py-2 last-of-type:hidden">
              <SfLink :tag="NuxtLink" :to="item.link" variant="secondary"
      ...
      <li v-for="(item, index) in breadcrumbs" :key="item.name"
        class="peer text-lg hidden sm:flex items-center peer-[:nth-of-type(even)]:before:content-['/'] peer-[:nth-of-type(even)]:before:px-2 peer-[:nth-of-type(even)]:before:leading-5 last-of-type:flex last-of-type:before:font-normal last-of-type:before:text-neutral-500 text-neutral-500 last-of-type:text-neutral-900 last-of-type:font-medium">
        <SfLink v-if="index < breadcrumbs.length - 1" :tag="item.link ? NuxtLink : 'div'" :to="item.link"
          variant="secondary"
          class="leading-5 text-yellow-500 no-underline hover:underline active:underline whitespace-nowrap outline-secondary-600 text-inherit">
          {{ item.name }}
        </SfLink>
